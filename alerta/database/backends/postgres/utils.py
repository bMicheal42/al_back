...
class Alerts(QueryBuilder):
    ...
    def from_params(params: MultiDict, customers=None, query_time=None):
        ...
        if len(ids) == 1:
            query.append('AND alerts.id LIKE %(id)s')
            qvars['id'] = ids[0] + '%'
        elif ids:
            query.append('AND id ~* (%(regex_id)s)')
            qvars['regex_id'] = '|'.join(['^' + i for i in ids])

        ackedBy = params.get('acked-by', None)
        if ackedBy:
            query.append("AND (attributes->>'acked-by' = %(acked_by)s OR attributes->>'acked-by' IS NULL)")
            qvars['acked_by'] = ackedBy
